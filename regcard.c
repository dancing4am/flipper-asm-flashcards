...
static void setup_quiz(AppState* state) {
    int count = decks[state->active_deck].count;
...
static void draw_callback(Canvas* canvas, void* context) {
    ...
    case ScreenTitle:
        ...
        canvas_draw_str(canvas, 10, 26, "Register Cards");
    ...
    case ScreenMenu:
        canvas_set_font(canvas, FontSecondary);
        for(int i = 0; i < (int)COUNT(decks); i++) {
            if(i == state->selected) {
                canvas_draw_str(canvas, 2, 12 + i * 9, ">");
            }
            canvas_draw_str(canvas, 12, 12 + i * 9, decks[i].label);
        }
        break;
    case ScreenMode:
        canvas_set_font(canvas, FontSecondary);
        for(int i = 0; i < (int)COUNT(modes); i++) {
            if(i == state->mode_cursor) {
                canvas_draw_str(canvas, 2, 12 + i * 9, ">");
            }
            canvas_draw_str(canvas, 12, 12 + i * 9, modes[i].label);
        }
        break;
    case ScreenCard: {
        int card_index = state->order[state->position];
        const Card* card = &decks[state->active_deck].cards[card_index];
        canvas_set_font(canvas, FontPrimary);
    ...
    case ScreenQuiz: {
        ...
        const Card* q = &decks[state->active_deck].cards[q_index];
        ...
        if(state->quiz_phase == PHASE_CORRECT) {
            ...
            for(int i = 0; i < 2; i++) {
                const Card* o = &decks[state->active_deck].cards[state->opt[i]];
    ...
    while(running) {
        if(furi_message_queue_get(queue, &event, FuriWaitForever) == FuriStatusOk) {
            if(event.type == InputTypePress) {
                switch(event.key) {
                case InputKeyBack:
                    if(state.screen == ScreenTitle || state.screen == ScreenMenu) {
                        running = false;
                ...
                case InputKeyOk:
                    if(state.screen == ScreenTitle) {
                        ...
                        shuffle_deck(&state, decks[state.active_deck].count);
                        state.screen = ScreenMode;
                        state.mode_cursor = 0;
                    } else if(state.screen == ScreenMode) {
                        state.position = 0;
                        ...
                        if(state.quiz_phase == PHASE_ANSWER) {
                            ...
                            if(state.position >= decks[state.active_deck].count) {
                                shuffle_deck(&state, decks[state.active_deck].count);
                            }
                            setup_quiz(&state);
                        } 
                ...
                case InputKeyDown:
                    if(state.screen == ScreenMenu) {
                        if(state.selected != COUNT(decks) - 1) {
                            state.selected = state.selected + 1;
                        } else {
                            state.selected = 0;
                        }
